...
@Component
public class CreateOutlineMediaCommentRoute implements Routable {
    @Autowired
    ...
    public Function<PredicateSpec, Buildable<Route>> getRoute() {
        return predicateSpec -> predicateSpec.path(gatewayProperties.getCreateOutlineMediaCommentPath())
                .and()
                .method(HttpMethod.POST)
                .filters(gatewayFilterSpec -> gatewayFilterSpec.stripPrefix(2)
                        .prefixPath(serviceProperties.getCreateOutlineMediaCommentPath()))
