...
public class GetOutlineMediaCommentByOutlineMediaRoute implements Routable {
    ...
    public Function<PredicateSpec, Buildable<Route>> getRoute() {
        return predicateSpec -> predicateSpec.path(gatewayProperties.getGetOutlineMediaCommentsByOutlineMediaPath())
                .and()
                .method(HttpMethod.GET)
                .filters(gatewayFilterSpec -> gatewayFilterSpec.stripPrefix(2)
                        .prefixPath(serviceProperties.getGetOutlineMediaCommentsByOutlineMediaPath()))
