...
@Component
public class GetSpecificLessonsByGroupIdRoute implements Routable {
    @Autowired
    ...
    public Function<PredicateSpec, Buildable<Route>> getRoute() {
        return predicateSpec -> predicateSpec.path(gatewayResourceProviderPathProperties.getGetSpecificLessonsByGroupIdPath())
                .and()
                .method(HttpMethod.GET)
                .filters(gatewayFilterSpec -> gatewayFilterSpec.stripPrefix(2)
                        .prefixPath(serviceProperties.getGetSpecificLessonsByGroupIdPath()))
