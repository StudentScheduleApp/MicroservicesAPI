...
@Component
public class GetUserByTokenRoute implements Routable {
    @Autowired
    ...
    public Function<PredicateSpec, Buildable<Route>> getRoute() {
        return predicateSpec -> predicateSpec.path(gatewayIdentityPathProperties.getGetUserIdByTokenPath())
                .and()
                .method(HttpMethod.POST)
                .filters(gatewayFilterSpec -> gatewayFilterSpec.stripPrefix(2)
                        .prefixPath(identityServiceProperties.getGetUserIdByTokenPath()))
