...
@Component
public class GetOutlineMediaCommentByIdRoute implements Routable {
    @Autowired
    ...
    public Function<PredicateSpec, Buildable<Route>> getRoute() {
        return predicateSpec -> predicateSpec.path(gatewayProperties.getGetOutlineMediaCommentByIdPath())
                .and()
                .method(HttpMethod.GET)
                .filters(gatewayFilterSpec -> gatewayFilterSpec.stripPrefix(2)
                        .prefixPath(serviceProperties.getGetOutlineMediaCommentByIdPath()))
