...
@Component
public class CreateScheduleTemplateRoute implements Routable {
    @Autowired
    ...
    public Function<PredicateSpec, Buildable<Route>> getRoute() {
        return predicateSpec -> predicateSpec.path(gatewayProperties.getCreateScheduleTemplatePath())
                .and()
                .method(HttpMethod.POST)
                .filters(gatewayFilterSpec -> gatewayFilterSpec.stripPrefix(2)
                        .prefixPath(serviceProperties.getCreateScheduleTemplatePath()))
